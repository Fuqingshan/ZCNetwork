...
/**
 ZCPinningModeDefault：App Transport Security Settings -> Allow Arbitrary Loads -> YES
...
typedef NS_ENUM(NSInteger,ZCPinningMode) {
    ZCPinningModeDefault = -1,///<验证无效证书，验证域名，AFSSLPinningModeNone，（AFHTTPSessionManager默认参数，没有导入证书也无法抓包）
...
/**
 1、[[ZCHTTPSessionManager alloc] init]始化默认None
 2、图片等三方资源如果支持https，也要加到Exception Domains中
 3、不用init初始化默认ZCPinningModeDefault
 */
@property (nonatomic, assign) ZCPinningMode mode;
